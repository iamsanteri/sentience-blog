        ...
        <>
            <section className="lg:pt-4 pb-12 feed-group">
                <div className="mx-7 lg:mx-32 lg:px-32 xl:mx-64 xl:px-42 2xl:mx-72 2xl:px-48">
                    {filteredList.map(({ id, type, title }) => (
                        <div key={id} className="group">
                            <Link className="flex justify-between" href={`/${type}/${id}`}>
                                <span className="flex text-lg py-3 dark:text-white group-hover:opacity-75 santeri">{ title }</span>
                                <div className="relative flex items-center space-x-1">
                                    <span className="text-base py-3 text-gray-500 dark:text-gray-400 mt-0.5 transition-all duration-300 ease-out group-hover:mr-4">{ id }</span>
